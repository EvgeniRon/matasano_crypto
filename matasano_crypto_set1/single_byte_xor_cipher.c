...
* 	Decrypting the cipher using all keys (byte - 256 options - ascii table). 
*	Than scoring each plaintext by calculating the distance between the observed probability of a 
*	letter and the letter frequency in the English language. 
*	Also, no observation of a letter is penalized. Second method is to choose the key 
*	that generates the highest count of letters.
*	User can choose scoring method by setting distance_scoring : 0 - for distance scoring, 1 -  for maximum letters count.
*/

u_int8 *break_cipher(const char *ct, int penalty, int distance_scoring ){

	float sum = 0;
	float score[ASCII_SYMBOLS] = {0};
	unsigned int letter_freq_cp[ASCII_SYMBOLS][ENGLISH_LETTERS]={0};
	unsigned int max_letters_key = 0;
	unsigned int min_distance_key = 0;
	unsigned int total_num_letters[ASCII_SYMBOLS];
	unsigned int key = 0;
	unsigned int col;
	unsigned int decypher_key;
	unsigned long str_len = strlen(ct);

	/*	Allocating byte array for convertion between hex string to byte array	*/
	u_int8 *byteArr = (u_int8 *)malloc(str_len/2);
	if (byteArr == NULL){
		free(byteArr);
	...
	/* Decrypting the cipher with each ascii symbol and counting the letter frequncy for each key */
	...
	if (distance_scoring){  // Scoring method choise
		decypher_key = min_distance_key;
	} else {
		decypher_key = max_letters_key;
	}

	for ( int i = 0 ; i< str_len; i++)  // Decrypting with the chosen key
		byteArr[i] = byteArr[i] ^ decypher_key;
